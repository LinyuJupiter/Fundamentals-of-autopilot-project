...
%% Simulation lenght and plotting 模拟长度和绘图
simN = 500;
...
%% Fit spline to track 将样条曲线拟合到轨迹
...
%% Define starting position 定义起始位置
...
%% Initialize logging arrays 初始化日志记录阵列
...
for i = 1: simN
    ...
    % augment state and inputs by shifting previus optimal solution 通过转移先验最优解来增加状态和输入
    [x,u] = augState(x,u,x0,MPC_vars,ModelParams,tl);
    %  formulate MPCC problem and solve it 制定MPCC问题并加以解决
    if QP_iter == 0
